...
func (r *userRepo) GetByID(ctx context.Context, id string) (*model.User, error) {
    q := `SELECT id, name, email, password, role, created_at FROM users WHERE id = $1 LIMIT 1`
    var u model.User
    err := r.db.QueryRowContext(ctx, q, id).Scan(&u.ID, &u.Name, &u.Email, &u.Password, &u.Role, &u.CreatedAt)
    if err != nil {
        if errors.Is(err, sql.ErrNoRows) {
            return nil, nil
        }
