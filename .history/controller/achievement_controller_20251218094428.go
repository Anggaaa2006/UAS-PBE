...
/*
	GET /achievements
	List prestasi berdasarkan role user
*/
...
*/
func (c *AchievementController) History(ctx *gin.Context) {

	id := ctx.Param("id")

	data, err := c.svc.GetHistory(ctx.Request.Context(), id)
	if err != nil {
		utils.Error(ctx, 500, err.Error())
		return
	}
...
func (c *AchievementController) AddAttachment(ctx *gin.Context) {
	id := ctx.Param("id")
	var req struct {
	...
	if err := c.svc.AddAttachment(ctx, id, req.FileURL); err != nil {
		utils.Error(ctx, 500, err.Error())
		return
	}
	utils.Success(ctx, "attachment added")
}

func (c *AchievementController) GetAttachments(ctx *gin.Context) {
	id := ctx.Param("id")
	data, err := c.svc.GetAttachments(ctx, id)
	if err != nil {
		utils.Error(ctx, 500, err.Error())
		return
	}
